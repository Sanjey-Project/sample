...
// Index the document
$result = $es->index(INDEX_SUBJECTS, $newId, $document);
            ...
            if($es->indexExists(INDEX_SUBJECTS)) {
                $checkQuery = [
                    'query' => [
                        'bool' => [
                            'must' => [
                                ['term' => ['subjectName.keyword' => $subjectname]],
                ...
                if($checkResult['success']) {
                    ...
                    if($totalHits > 0 && isset($checkResult['data']['hits']['hits'][0])) {
                        $subjectExists = true;
                        $existingSubjectId = $checkResult['data']['hits']['hits'][0]['_source']['id'];
                    }
                }
            }
    
            if($subjectExists && $existingSubjectId) {
                ...
                $updateResult = $es->index(INDEX_SUBJECTS, $existingSubjectId, $updateDoc);
                ...
                $insertResult = $es->index(INDEX_SUBJECTS, $newId, $document);
